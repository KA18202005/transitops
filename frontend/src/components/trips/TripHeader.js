...
export default function TripHeader({ onCreate }) {
  return (
    <div className="flex flex-col gap-4 rounded-[32px] border border-slate-200 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 p-6 text-white shadow-xl lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h1 className="text-3xl font-semibold tracking-tight">Trip Management</h1>
        <p className="mt-3 max-w-2xl text-sm text-slate-300">
          Plan, dispatch, monitor, complete, and cancel transport operations.
        </p>
      </div>
      <button type="button" onClick={onCreate} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-4 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-100">
        <Send size={18} />
        Create Trip
      </button>
    </div>
